...
if ($form_name == 'add_spk') {
	...
	$sql001 = mysqli_query($conn, "SELECT * FROM user WHERE posisi_user = 'Teknisi' ORDER BY nama_user ASC");
	while ($row_teknisi = $sql001->fetch_assoc()) {
		$idTek = 'teknisi' . $row_teknisi['id_user'] . '_spk';
		$z = 0;
		if (isset($_POST[$idTek])) {
			$productteknisi[$z] = mysqli_real_escape_string($conn, trim($_POST[$idTek]));
			$dataTek = $dataTek . $productteknisi[$z] . ';';
		}
		$z++;
	}
...
if ($form_name == 'edit_spk') {
	...
	$sql001 = mysqli_query($conn, "SELECT * FROM user WHERE posisi_user = 'Teknisi'");
	while ($row_teknisi = $sql001->fetch_assoc()) {
		$idTek = 'teknisi' . $row_teknisi['id_user'] . '_spk';
		$z = 0;
		if (isset($_POST[$idTek])) {
	...
	// user
	// if (isset($_POST['teknisi30_spk'])) {
	// 	$productteknisi30 = mysqli_real_escape_string($conn, trim($_POST['teknisi30_spk']));
	// 	$dataTek = $dataTek . $productteknisi30 . ';';
	// }
	// if (isset($_POST['teknisi31_spk'])) {
	// 	$productteknisi31 = mysqli_real_escape_string($conn, trim($_POST['teknisi31_spk']));
	// 	$dataTek = $dataTek . $productteknisi31 . ';';
	// }
	// if (isset($_POST['teknisi32_spk'])) {
	// 	$productteknisi32 = mysqli_real_escape_string($conn, trim($_POST['teknisi32_spk']));
	// 	$dataTek = $dataTek . $productteknisi32 . ';';
	// }
	// if (isset($_POST['teknisi33_spk'])) {
	// 	$productteknisi33 = mysqli_real_escape_string($conn, trim($_POST['teknisi33_spk']));
	// 	$dataTek = $dataTek . $productteknisi33 . ';';
	// }
	// if (isset($_POST['teknisi34_spk'])) {
	// 	$productteknisi34 = mysqli_real_escape_string($conn, trim($_POST['teknisi34_spk']));
	// 	$dataTek = $dataTek . $productteknisi34 . ';';
	// }
	// if (isset($_POST['teknisi36_spk'])) {
	// 	$productteknisi36 = mysqli_real_escape_string($conn, trim($_POST['teknisi36_spk']));
	// 	$dataTek = $dataTek . $productteknisi36 . ';';
	// }
	// if (isset($_POST['teknisi37_spk'])) {
	// 	$productteknisi37 = mysqli_real_escape_string($conn, trim($_POST['teknisi37_spk']));
	// 	$dataTek = $dataTek . $productteknisi37 . ';';
	// }
	// if (isset($_POST['teknisi38_spk'])) {
	// 	$productteknisi38 = mysqli_real_escape_string($conn, trim($_POST['teknisi38_spk']));
	// 	$dataTek = $dataTek . $productteknisi38 . ';';
	// }
...
if ($form_name == 'edit_stiker') {
	// $productid_p = mysqli_real_escape_string($conn, trim($_POST['id_stiker']));
	$productkode_p = mysqli_real_escape_string($conn, trim($_POST['kode_stiker']));

	$code_huruf = substr($productkode_p, 0, 1);
	$code_angka = (int) substr($productkode_p, 1, 5);
	$code_angka = $code_angka + 41;
	$var_angka = sprintf("%'.05d", $code_angka);
	$status = 'merah';
	...
	if ($status == 'hijau') {
		$queryH = "SELECT * FROM stiker WHERE huruf_stiker='H'";
		$resultH = mysqli_query($conn, $queryH) or die(mysqli_error($conn));
		$rowH = mysqli_fetch_assoc($resultH);
		$angkaH = (int) $rowH['angka_stiker'];

		if ($angkaH < $code_angka) {
			$query = "UPDATE stiker SET angka_stiker='$var_angka' WHERE huruf_stiker='H'";
			$result = mysqli_query($conn, $query) or die(mysqli_error($conn));
		...
		$queryM = "SELECT * FROM stiker WHERE huruf_stiker='M'";
		$resultM = mysqli_query($conn, $queryM) or die(mysqli_error($conn));
		$rowM = mysqli_fetch_assoc($resultM);
		$angkaM = (int) $rowM['angka_stiker'];
		$varM = sprintf("%'.05d", $angkaM++);

		if ($angkaM < $code_angka) {
			$query = "UPDATE stiker SET angka_stiker='$var_angka' WHERE huruf_stiker='M'";
			$result = mysqli_query($conn, $query) or die(mysqli_error($conn));
