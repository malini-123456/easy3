							...
							<div class="nk-content-body">
								<div>
									<ul class="nav nav-tabs nav-tabs-s2">
										<li class="nav-item">
											<a class="nav-link active" data-toggle="tab" href="#" onclick="proyekdetail_rev(<?= $proyekid; ?>)">PROJECT DETAIL</a>
										</li>
										<li class="nav-item">
													...
													<div class="col-6">
														<div class="card h-100">
															<div class="d-flex bg-indigo text-white">
																...
																<div class="p-2">
																	<!-- <a href="#" onclick="pelanggan_qr('<?= $row1['nama_pelanggan']; ?>')" class="toggle btn btn-qrPel btn-round btn-icon btn-xs btn-light"><em class="icon ni ni-qr"></em></a> -->
																	<!-- if ($proyekStatus != 'SELESAI' &&  $proyekStatus != 'BATAL') { -->
																	<a href="#" onclick="pelanggan_pdf('<?= $proyekid; ?>')" class="toggle btn btn-qrPel btn-round btn-icon btn-xs btn-light"><em class="icon ni ni-file-pdf"></em></a>
																	<a href=" #" id="<?= $idpelanggan; ?>" data-target="addPelanggan" data-dismiss="modal" class="toggle btn btn-editPel btn-round btn-icon btn-xs btn-light"><em class="icon ni ni-edit-fill"></em></a>
																</div>
															</div>
															<div class="card-inner">
																<dl class="row">
																	...
																	<dt class="col-sm-5">Kode Pelanggan</dt>
																	<dd class="col-sm-7"><?= $row1['id_pelanggan']; ?>
																	</dd>
																	<dt class="col-sm-5">Alamat</dt>
																	<dd class="col-sm-7">
																		<?= $row1['alamat_pelanggan']; ?></dd>
																	<dt class="col-sm-5">NPWP</dt>
																	<dd class="col-sm-7"><?= $row1['npwp_pelanggan']; ?>
																	</dd>
																	<dt class="col-sm-5">Nama NPWP</dt>
																	<dd class="col-sm-7">
																		<?= $row1['namanpwp_pelanggan']; ?></dd>
																	<dt class="col-sm-5">PIC Keuangan</dt>
																	<dd class="col-sm-7">
																		<?= $row1['keuangan_pelanggan']; ?></dd>
																	<dt class="col-sm-5">PIC Teknis</dt>
																	<dd class="col-sm-7">
																		<?= $row1['teknis_pelanggan']; ?></dd>
																	<dt class="col-sm-5">Karakteristik</dt>
																	<dd class="col-sm-7">
																		<?= $row1['catatan_pelanggan']; ?></dd>
																</dl>
														...
														<div class="card h-100">
															<div class="d-flex bg-indigo text-white">
																...
																<div class="p-2">
																	...
																	if ($proyekStatus != 'SELESAI' &&  $proyekStatus != 'BATAL') {
																	?>
																		<h6><a href=" #" id="<?= $proyekid; ?>" data-target="addProyek" data-dismiss="modal" class="toggle btn btn-edit btn-round btn-icon btn-xs btn-light"><em class="icon ni ni-edit-fill"></em></a></h6>
																	<?php } ?>
																</div>
															</div>
															<div class="card-inner">
																<dl class="row">
																	...
																	<dd class="col-sm-7">
																		<?= date("d F Y", strtotime($row0['tglorder_proyek'])); ?></dd>
																	<dt class="col-sm-5">Marketing</dt>
																	<dd class="col-sm-7">
																		<?= $row0['marketing_proyek']; ?></dd>
																	<dt class="col-sm-5">Nama Pemilik Alat</dt>
																	<dd class="col-sm-7">
																		<?= $row0['namapemilik_proyek']; ?></dd>
																	<dt class="col-sm-5">Alamat Pemilik Alat</dt>
																	<dd class="col-sm-7"><?= $row0['alamatpemilik_proyek']; ?></dd>
																	<dt class="col-sm-5">Permohonan Melalui</dt>
																	<dd class="col-sm-7"><?= $row0['permohonan_proyek']; ?></dd>
																	<dt class="col-sm-5">Catatan</dt>
																	<dd class="col-sm-7"><?= $row0['catatan_proyek']; ?>
																	</dd>
																</dl>
																<!-- <div class="d-flex flex-row-reverse">
                                                                    <a href="#" id="<?= $proyekid; ?>" data-target="addProyek" data-dismiss="modal" class="toggle btn btn-edit btn-warning"><em class="icon ni ni-edit-fill"></em><span>Edit Proyek</span></a>
                                                                </div> -->
															</div>
														</div>
													</div>
													...
													<div class="card-inner">
														<table class="datatable-init-export_action1asc wrap table fs-12px" data-export-title="Export">
															...
															<tbody>
																<?php
																$sno = 0;
																$query = "SELECT * FROM proyek WHERE id_pelanggan = '$idpelanggan' ORDER BY id_proyek DESC";
																...
																?>
																	<tr>
																		<!-- <td><?= $sno; ?></td> -->
																		<td><?= $row['no_proyek']; ?></td>
																		<td><?= date("d F Y", strtotime($row['tglorder_proyek'])); ?></td>

																		<!-- Jumlah Alat Bisa Dikerjakan -->
																		<?php
																		$totalMampu = 0;
																		$totalTidakMampu = 0;
																		$query2 = "SELECT * FROM kajiulang WHERE id_proyek = " . $row['id_proyek'] . " ORDER BY id_kajiulang DESC";
																		$result2 = mysqli_query($conn, $query2) or die(mysqli_error($conn));
																		while ($row2 = mysqli_fetch_array($result2)) {

																			$id_layanan = $row2['id_layanan'];
																			$query1 = "SELECT * FROM layanan WHERE id_layanan = '$id_layanan'";
																			$result1 = mysqli_query($conn, $query1) or die(mysqli_error($conn));
																			$row1 = mysqli_fetch_assoc($result1);

																			if ($row2['kategori_kajiulang'] != 'Tidak Bisa Dikalibrasi')
																				$totalMampu = $totalMampu + $row2['jumlah_barang_kajiulang'];
																		}
																		...
																		<?php } else { ?>
																			<td class="text-center"><?= $totalMampu; ?></td>
																		<?php } ?>
																		...
																		?>
																			...
																			<td class="text-center"><?= $row_tekkoor['nama_user']; ?></td>
																		<?php
																		} else {
																		?>
																			<td>-</td>
																		...
																		<?php if ($row['status_proyek'] == 'ON GOING') { ?>
																			<td class="text-center"><span class="badge badge-dot badge-success"><?= $row['status_proyek']; ?></span></td>
																		<?php } elseif ($row['status_proyek'] == 'BATAL') { ?>
																			<td class="text-center"><span class="badge badge-dot badge-danger"><?= $row['status_proyek']; ?></span></td>
																		<?php } elseif ($row['status_proyek'] == 'SELESAI') { ?>
																			<td class="text-center"><span class="badge badge-dot badge-primary"><?= $row['status_proyek']; ?></span></td>
																		<?php } else { ?>
																			<td class="text-center"> <span class="badge badge-dot badge-warning"><?= $row['status_proyek']; ?></span></td>
																		<?php } ?>
																	</tr>
																<?php
																}
